...
the upper-snake-case fallback for anything not in the map — so the upcoming
restructure into liftostrava/strava/exercise_map.py can't silently change
either.
"""
...
def test_known_mapping(name, expected):
    assert sts.exercise_type_for(name) == expected


def test_unmapped_name_falls_back_to_upper_snake_case():
    assert sts.exercise_type_for("Some New Machine") == "SOME_NEW_MACHINE"


def test_fallback_collapses_punctuation_runs_to_a_single_underscore():
    # A regression guard for the bug described at sync_to_strava.py's
    ...
    assert sts.exercise_type_for("Some New, Cable Machine") == "SOME_NEW_CABLE_MACHINE"


def test_fallback_strips_leading_and_trailing_punctuation_and_whitespace():
    assert sts.exercise_type_for("  Weird!! Name--  ") == "WEIRD_NAME"
