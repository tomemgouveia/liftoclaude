...
transformation from a Workout into the JSON Strava's /uploads endpoint
expects. In particular, default_utc_offset's DST handling is a regression
guard: an activity uploaded without a correct utc_offset previously
rendered an hour early on Strava whenever the athlete wasn't literally in
UTC+0.
"""
...
def test_default_utc_offset_accounts_for_dst(iso, expected_offset):
    assert default_utc_offset(iso) == expected_offset


def test_build_strava_payload_flattens_every_set_across_exercises(sample_workout):
    payload = build_strava_payload(sample_workout)
...
def test_build_strava_payload_computes_utc_offset_when_absent(sample_workout):
    assert sample_workout.utc_offset is None  # sanity-check the fixture
    payload = build_strava_payload(sample_workout)
    assert payload["utc_offset"] == default_utc_offset(sample_workout.start_time)
